...
import { Send, Smile, Reply, Hash, MessageSquarePlus, MessageSquare } from "lucide-react";
...
function ThreadsButton({
  ...
  const list = (threads ?? []) as ThreadWithMeta[];
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent side="bottom" align="end" className="w-80 p-0">
        <div className="p-2 border-b flex items-center justify-between">
          <span className="text-sm font-semibold">Tópicos do grupo</span>
          <Button
            type="button"
            size="sm"
            variant="ghost"
            ...
            list.map((t) => (
              <button
                ...
                className={cn(
                  ...
                  activeThreadId === t.id && "bg-muted/50",
                )}
              >
