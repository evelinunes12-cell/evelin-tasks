...
BEGIN
  ...
  -- ========== COHORT RETENTION ==========
  -- For each week in the range, calculate % of users who were active after 1, 7, 30 days
  ...
  FROM (
    SELECT
      ...
      ROUND(count(DISTINCT CASE WHEN p.last_activity_date >= (p.created_at::date + 1) THEN p.id END)::numeric 
            / NULLIF(count(DISTINCT p.id), 0) * 100, 1) AS retention_d1,
      ROUND(count(DISTINCT CASE WHEN p.last_activity_date >= (p.created_at::date + 7) THEN p.id END)::numeric 
            / NULLIF(count(DISTINCT p.id), 0) * 100, 1) AS retention_d7,
      ROUND(count(DISTINCT CASE WHEN p.last_activity_date >= (p.created_at::date + 30) THEN p.id END)::numeric 
  ...
  -- ========== USAGE HEATMAP ==========
  -- Count activity (tasks updated + focus sessions) by day_of_week (0=Sun) and hour
  ...
  -- ========== CHURN RATE ==========
  -- Weekly snapshot: how many users inactive for 7, 14, 30 days
  SELECT coalesce(jsonb_agg(row_to_json(cr)::jsonb ORDER BY cr.week_date), '[]'::jsonb)
  INTO v_churn_rate
  FROM (
    SELECT
      ws.week_date::text AS week_date,
      (SELECT count(*) FROM public.profiles p 
       WHERE p.is_active = true 
         AND p.created_at::date <= ws.week_date
         AND (p.last_activity_date IS NULL OR p.last_activity_date < ws.week_date - 7)
      ) AS inactive_7d,
      (SELECT count(*) FROM public.profiles p 
       WHERE p.is_active = true 
         AND p.created_at::date <= ws.week_date
         AND (p.last_activity_date IS NULL OR p.last_activity_date < ws.week_date - 14)
      ) AS inactive_14d,
      (SELECT count(*) FROM public.profiles p 
       WHERE p.is_active = true 
         AND p.created_at::date <= ws.week_date
         AND (p.last_activity_date IS NULL OR p.last_activity_date < ws.week_date - 30)
      ) AS inactive_30d
